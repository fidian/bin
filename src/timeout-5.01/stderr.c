...
@(#)Version:        $Revision: 10.8 $
@(#)Last changed:   $Date: 2013/05/10 00:13:29 $
@(#)Purpose:        Error reporting routines
@(#)Author:         J Leffler
@(#)Copyright:      (C) JLSS 1988-91,1996-99,2001,2003,2005-11,2013
@(#)Product:        TIMEOUT Version 5.01 (2014-12-15)
*/
...
const char jlss_id_stderr_c[] = "@(#)$Id: stderr.c,v 10.8 2013/05/10 00:13:29 jleffler Exp $";
#endif /* lint */
...
const char *(err_rcs_string)(const char *s2, char *buffer, size_t buflen)
{
    ...
    ** "$Revision: 10.8 $ ($Date: 2013/05/10 00:13:29 $)"
...
static void (err_filedes)(int fd, int flags, int errnum, const char *format, va_list args)
{
    ...
    nbytes = write(fd, buffer, curpos - buffer);
